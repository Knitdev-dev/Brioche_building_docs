...
import { solve, HARNESS, NO2_ANCHOR } from "./brioche-calculator.js";
...
for (const [name, args] of Object.entries(HARNESS)) {
  const r = solve(args);
  if (!r.ok) {
    console.log(`${name}: NO SOLUTION diagnostics=`, r.diagnostics);
    allOk = false;
    continue;
  }
  ...
  if (name === "No2") {
    for (const [field, target] of Object.entries(NO2_ANCHOR)) {
      ...
      console.log(`      anchor check ${field}: target=${target} achieved=${achieved} gap=${gap} [${gap === 0 ? "OK" : "MISS"}]`);
